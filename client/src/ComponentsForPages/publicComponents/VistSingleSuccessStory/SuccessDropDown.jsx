...
import SuccessLandingHeader from './SuccessLandingHeader'

const SuccessDropDown = ({ newGif, newGif2, darkMode, setDarkMode, openMenu, setOpenMenu, landingContent, allStories, allNewsLetters, singleStory }) => {
  ...
  return (
    ...
    <div >
      ...
      <SuccessLandingHeader darkMode={darkMode} setDarkMode={setDarkMode} singleStory={singleStory} newGif={newGif} newGif2={newGif2} />
          ...
          {openMenu === "success" && (
            <div className="dropdownPanel" style={{ background: "white", border: "2px solid black" }} >
              ...
              <>
                ...
                {publishedStories?.filter(story => story?._id !== singleStory?._id)?.length === 0 ? "Currently No Success Stories To View" :
                  <>
                    {visibleStories.filter(story => story?._id !== singleStory?._id)?.map(story => (
                      <Link to={`/successStory/${story?._id}`}>
