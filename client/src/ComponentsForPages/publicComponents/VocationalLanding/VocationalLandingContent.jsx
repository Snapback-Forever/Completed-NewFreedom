...
const VocationalLandingContent = ({ darkMode, setDarkMode, allSupporters, allStories }) => {
  ...
  return ( 
    ...
    <div style={{ height: "84.1vh", overflow: "scroll", display: "flex", flexDirection: "column", alignItems: "center" }} className={darkMode ? "scrollBar QADark" : "scrollBar QAWhite"} >
      ...
      {allPrograms?.filter(pro => pro?.programType === "vocational").length === 0 ? <h2 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>Currently No Vocational Programs To View</h2> :
        <>
          <h1 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>Vocational Programs</h1>
          {
            allPrograms?.filter(pro => pro?.programType === "vocational").map(pro => {
              const baseUrl = "http://localhost:8080";
              const imgSrc =
                pro?.imageFileId && pro?.imageBucketName
                  ? `${baseUrl}/upload/image/${pro.imageFileId}?bucketName=${pro.imageBucketName}`
                  : pro?.link;
        ...
        <div style={{ width: "100vw", display: "flex", overflowX: "auto", gap: "1vw", boxSizing: "border-box", margin: "1vh 0" }}>
          {allStories
              ...
              return (
                <Link key={story?._id || index} to={`/successStory/${story?._id}`} style={{ color: "black", textDecoration: "none", flexShrink: 0 }}>
                  <div style={{ minWidth: "40vw", maxWidth: "50vw", minHeight: "40vh", maxHeight: "40vh", margin: "0.5vh 0.5vw", background: "white", flexShrink: 0 }}>
                    ...
                    <h5 style={{ textAlign: "center", whiteSpace: "pre-wrap", }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(story?.outcomeSummary) }} />
                  </div>
                </Link>
              );
            })}
...
export default VocationalLandingContent
