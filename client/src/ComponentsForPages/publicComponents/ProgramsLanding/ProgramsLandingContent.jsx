...
const ProgramsLandingContent = ({ darkMode, setDarkMode,  allSupporters, allStories }) => {
  ...
  return (
    ...
    <div  style={{ height: "84.1vh", overflow: "scroll", display: "flex", flexDirection: "column", alignItems: "center" }} className={ darkMode ? "scrollBar QADark" : "scrollBar QAWhite" } >
      ...
      {allPrograms?.filter(pro => pro?.programType === "reg-Program").length === 0 ? <h1 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>Currently No Programs To View</h1> :
        <>
        <h1 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>Non Vocational Programs</h1>
          {
            allPrograms?.filter(pro => pro?.programType === "reg-Program").map(pro => {
             
              return (
                <div style={{ background: "rgba(250, 235, 215, 0.960)", width: "98vw", margin: "1vh 1vw", padding: "1vh" }} key={crypto.randomUUID()}>
...
export default ProgramsLandingContent
