...
const VisitSingleNewsDropDown = ({ newGif, newGif2, darkMode, setDarkMode, openMenu, setOpenMenu, landingContent, allStories, allNewsLetters, singleNews }) => {
        ...
        >
          ...
          {openMenu === "news" && (
            ...
            <div className="dropdownPanel" style={{ background: "white", border: "2px solid black" }} >
              ...
              {allNewsLetters?.filter(news => news?._id !== singleNews?._id).length === 0 ? "Currently No Newsletters To View" :
                <>
                  {visibleNewsLetters?.filter(news => news?._id !== singleNews?._id).map(news => (
...
export default VisitSingleNewsDropDown
