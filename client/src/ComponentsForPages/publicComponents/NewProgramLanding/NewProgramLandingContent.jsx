...
const NewProgramLandingContent = ({ darkMode, setDarkMode, allSupporters, allStories }) => {
  ...
  return (
    ...
    <div  style={{ height: "84.1vh", overflow: "scroll", display: "flex", flexDirection: "column", alignItems: "center" }} className={ darkMode ? "scrollBar QADark" : "scrollBar QAWhite" } >
      ...
      {allPrograms?.filter(pro => pro?.programType === "reg-Program" && pro?.createdAt && new Date(pro.createdAt) >= new Date(new Date().setMonth(new Date().getMonth() - 12))).length === 0 ? <h2 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>Currently No New Programs Added Within The Last Year To View</h2> :
        <>
        <h1 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>New Programs Added Within The Last Year </h1>
        <h2 style={{ width: "100%", textAlign: "center", color: darkMode ? "white" : "black" }}>- Non Vocational Programs</h2>
          {
            allPrograms?.filter(pro => pro?.programType === "reg-Program" && pro?.createdAt && new Date(pro.createdAt) >= new Date(new Date().setMonth(new Date().getMonth() - 12))).map(pro => {
...
export default NewProgramLandingContent
