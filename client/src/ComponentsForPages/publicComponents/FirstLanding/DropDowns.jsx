...
import FirstLandingPageHeader from './FirstLandingPageHeader'
import moment from 'moment'
import { Link } from 'react-router-dom'

const DropDowns = ({ newGif, newGif2, darkMode, setDarkMode, openMenu, setOpenMenu, landingContent, allStories, allNewsLetters }) => {
  ...
  return (
    ...
    <div >
      ...
      <FirstLandingPageHeader newGif={newGif} newGif2={newGif2} darkMode={darkMode} setDarkMode={setDarkMode} landingContent={landingContent} />
      ...
      >
        ...
        <div style={{ position: "relative", width: "100%", textAlign: "center", background: "goldenRod", margin: "0 1vw" }} className='rounded responsiveDiv' onMouseLeave={() => setOpenMenu("")}>
          ...
          {openMenu === "who" && (
            ...
            >
              <Link to={"/why"}>
                <button className='rounded buttHover' style={{ background: "goldenRod", width: "100%", margin: "0.5vh 0", color: "black" }}>Why We Exists</button>
              </Link>
                  ...
                  {visibleNewsLetters?.map(news => (
                    ...
                    <Link key={news?._id} to={`/newsletter/${news?._id}`}>
                      <button className="rounded" style={{ background: "goldenRod", width: "100%", margin: "0.5vh 0", color: "black" }}>
                        {news?.postTitle?.split(" ").slice(0, 8).join(" ") + (news?.postTitle?.split(" ").length > 8 ? "..." : "")}
                        : {moment(news?.periodStart).format("MMM YYYY")}</button>
                    </Link>
        ...
        >
          ...
          {openMenu === "support" && (
            ...
            >
              ...
              <a href='https://www.nfproject.org/'>
