...
const AdminMenteeCard = ({ mentee, imgSrcMentee, isBirthdayToday, setTrigger, allPrograms, allUsers }) => {
    ...
    const [showMoreMenteeListRemove, setShowMoreMenteeListRemove] = useState(false);
    const [addInformation, setAddInformation] = useState("");
    const [confirmStep, setConfirmStep] = useState(false);
    const [editMentee, setEditMentee] = useState(false);
    ...
    const handleUpdateDates = () => {
        ...
        if (
            currentForm.programStatus === "incarcerated" &&
            (!currentForm.projectedReleaseDate || !currentForm.maxReleaseDate)
        ) {
            return;
        }
        const payload = {
            mailId: mentee?._id,
            projectedReleaseDate: currentForm.projectedReleaseDate,
            maxReleaseDate: currentForm.maxReleaseDate,
            programStatus: currentForm.programStatus,
            userId: user?._id
        };
    ...
    const removeMentorFromUser = () => {
        setShowMoreMenteeListRemove("removeMentee")
    }

    const handleSubmit = (e, mentorId, menteeId) => {
        e.preventDefault();
        const payload = {
            mailId: menteeId,
            mentorId: mentorId,
    ...
    const imgSrcMentor =
        mentee.mentor?.profilePicFileId && mentee.mentor?.profilePicBucketName
            ? `${baseUrl}/upload/image/${mentee.mentor?.profilePicFileId}?bucketName=${mentee.mentor?.profilePicBucketName}`
            : mentee.mentor?.profilePic;

    return (
        <div key={mentee?._id} style={{ width: "90%", display: "flex", flexDirection: "column", border: "1px solid black", padding: "3vh 1vw", background: "white" }} className='responsiveAllMenteeCards'>
            {!mentee?.mentorAttached?.some(a => a?.active === true) && <h6 className='lookAtMe' style={{ textAlign: "center" }}>Has No Mentor</h6>}
            ...
            <div style={{ width: "100%", display: "flex", padding: "1vh 1vw", background: "white" }} className='responsiveAllMenteeCardDiv'>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
                    ...
                    <div style={{ background: "lightGrey", padding: "0 1vw" }}>
                        <h6><b>Program Status:</b> {mentee?.programStatus}</h6>
                        {mentee?.programStatus === "incarcerated" && (
                            <div>
                                <h6>
                                <b>Projected Release Date:</b>{" "}
                        ...
                        {editing && (
                            <div style={{ marginTop: "1rem", border: "double red", padding: "2vh 2vw" }}>
                                ...
                                <div>
                                    <label><b> Projected Release Date:</b>Update</label>
                                    <input
                                        type="date"
                                        name="projectedReleaseDate"
                                        value={form?.projectedReleaseDate || ""}
                                        onChange={handleChange}
                                    />

                                </div>
                                <div>
                                    <label><b>Update Max Release Date:</b></label>
                                    <input
                                        type="date"
                                        name="maxReleaseDate"
                                        value={form?.maxReleaseDate || ""}
                                        onChange={handleChange}
                                    />
                    ...
                    {mentee?.email ? <h6><b>Email:</b> {mentee?.email}</h6> : ""}
            ...
            {showMoreMentee ?
                ...
                <div style={{ width: "100%" }}>
                    ...
                    {mentee.rejectedAcceptance.isRejected ?
                        ...
                        <h6><b>Approved Acceptance:</b> {mentee?.approvedAcceptance?.isApproved ? "Approved" : "Processing"}</h6>
                    }

                    <h6><b>Required PaperWork:</b> {mentee?.completedRequiredPaperwork ? "Completed" : "Not Completed"}</h6>

                    {mentee.currentCharge ? <h6><b>Current Charge:</b> {mentee?.currentCharge}</h6> : <h6><b>Current Charges:</b> No Current Charges</h6>}
                        ...
                        {showMoreMenteeList === "paperWork" ?
                            <div>
                                ...
                                {mentee?.missingPaperwork?.map(paper => {
                                    return (
                                        <div key={paper?._id} style={{ display: "flex", flexDirection: "column", alignItems: "space-between", margin: "1vh 0", background: "white", padding: "0 1vw" }} className='responsiveAllMenteeContainer'>
                    ...
                    <div style={{ background: "lightGrey", padding: "0 1vw", margin: "1vh 0" }}>
                        ...
                        {showMoreMenteeList === "mentor" ? <h6 style={{ width: "100%", textAlign: "center", margin: "1vh 0" }} onClick={() => setShowMoreMenteeList("")}>Hide Mentor:</h6> : <h6 style={{ width: "100%", textAlign: "center", margin: "1vh 0" }} onClick={() => setShowMoreMenteeList("mentor")}>View Mentor: {!mentee?.mentor ? <span className='lookAtMe'>No Mentor Attached</span> : ""}</h6>}

                        {showMoreMenteeList === "mentor" ?
                            <>
                                {mentee?.mentor ? <div style={{ display: "flex", width: "100%" }}>
                                    <img src={imgSrcMentor} style={{ minWidth: "30%", maxWidth: "30%", minHeight: '30%', maxHeight: "30vh", margin: "1vh 1vw" }} />

                                    <div style={{ display: "flex", flexDirection: "column", width: "70%", justifyContent: "center", alignItems: "center" }}>
                                        <h6>{mentee?.mentor?._id !== user?._id ? <Link to={`/messagePage/${mentee?.mentor?._id}`}>{" "}{mentee?.mentor?.accountName}</Link> : "Your Mentee"}
                                        </h6>
                                        <h6><b>Mentor Email:</b> <br />{mentee?.mentor?.email}</h6>
                                        {mentee?.mentorAttached?.filter(mentor => mentor?.mentorId === mentee?.mentor?._id && mentor?.active).map(mentor => {
                                            return (
                                                <div key={mentor?._id}>
                                                    <span style={{ fontSize: "small" }}><b>Assigned Date:</b> <br />{moment(mentor?.assignedAt).format("hh:mm MMM Do YY")} </span>
                                                </div>
                                            )
                                        })}
                                    </div>
                                </div> : <h6>No Mentor Attached</h6>}

                                {mentee?.mentor ? <> <button className='responsiveAllMenteeCardButton' style={{ background: "red", border: confirmStep ? "solid darkRed" : "solid red", padding: "0 1vw", width: "100%", color: confirmStep ? "white" : "black" }} onClick={handleClick}>{confirmStep ? "Click again to confirm" : `Remove Mentee From Current Mentor`}</button></> : <button style={{ background: "lightBlue", padding: "0 2vw", margin: '1vh 0' }} onClick={() => setAddMentorModal(true)}>Add A mentor</button>}

                                <dialog open={addMentorModal} >
                                    <AdminMentorModal mentee={mentee} allUsers={allUsers} setAddMentorModal={setAddMentorModal} setTrigger={setTrigger} />
                                </dialog>

                                {showMoreMenteeListRemove === "removeMentee" &&
                                    <div style={{ border: "solid black", margin: "1vh 1vw", padding: "1vh 1vw", width: "100%" }}>


                                        <h4 style={{ textAlign: "center" }}>Reason For Removal Of Mentee</h4>
                                        <form onSubmit={(e) => handleSubmit(e, mentee?.mentor?._id, mentee?._id)} style={{ display: "flex", flexDirection: "column" }}>

                                            <textarea name="reason" value={state?.reason} onChange={handleChangeState} className="Reason For Removal" style={{ border: "solid lightGrey", background: "white", width: "100%", minHeight: "10vh" }} />

                                            <div style={{ display: "flex" }}>

                                                <div style={{ background: "red", color: "white", padding: "0 1vw", width: "30%", margin: "1vh 1vw" }} onClick={() => setShowMoreMenteeList("")}>Cancel</div>

                                                <button style={{ background: "green", color: "white", padding: "0 1vw", width: "70%", margin: "1vh 1vw" }} type="submit">Submit Reason </button>
                    ...
                    <div style={{ background: "lightGrey", padding: "0 1vw", margin: "1vh 0" }}>
                        ...
                        {showMoreMenteeList === "mentorMsg" ?
                            <>
                            {visibleMsgs?.length === 0 ? <h5>No Mentor Msg To show</h5> : <>
                                {visibleMsgs?.map((msg) => (
                                    <div key={msg?._id}>
            ...
            {user?.creator || user?.NFadmin || mentee?.mentorAttached?.some((m) => m?.mentorId === user?._id && m?.active === true) ?
                <div style={{ width: "100%", display: "flex", justifyContent: "end" }}>
                    <button style={{ background: "lightBlue", width: "fit-content", marginTop: "1vh", padding: "0 2vw" }} onClick={() => setEditMentee(true)}>Edit Mentee</button>
                </div> : ""}

            <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "end" }}>

                <dialog open={editMentee}>
                    <EditMenteeModal mentee={mentee} setTrigger={setTrigger} setEditMentee={setEditMentee} />
                </dialog>
...
export default AdminMenteeCard
