...
import { addLocationStaff, addTeacherToProgram } from '../../../../redux/reducers/locationReducer';
import { Link } from 'react-router-dom';

const AddStaffUserCard = ({ user, loc, setTrigger, setChangeContent }) => {
    
    const dispatch = useDispatch()

    const admin = useSelector(state => state.auth.user)

    const alreadyTeacher = user?.programsTeaching?.some(teach => (teach?._id || teach) === loc?._id);

    const baseUrl = "http://localhost:8080";
    const imgSrc = user?.profilePicFileId && user?.profilePicBucketName
  ...
  const selectThisTeacher = (userId) => {
    ...
    dispatch(addLocationStaff(form))
    setTrigger(true)
    setChangeContent("staff")
  }
...
export default AddStaffUserCard
