...
const AddImageModal = ({ singleProgram, setTrigger, setAddImage }) => {
    ...
    const handleUploadImage = (fieldPrefix) => (e) => {
        ...
        setForm((prev) => ({ ...prev, [previewKey]: previewUrl, [fileKey]: file }));
    };

    const handleSaveImage = async (e) => {
        e.preventDefault();
        const file = form.programImageFile;
        if (!file) return;
        ...
        if (cleanDescription) {
            cleanDescription = DOMPurify.sanitize(cleanDescription, { FORBID_TAGS: ["script", "iframe", "object", "embed", "form", "input", "button", "link", "meta", "base"], FORBID_ATTR: ["onerror", "onload", "onclick"] });
        }
        const payload = {
        ...
        setForm({ programImagePreview: null, programImageFile: null, link: "", description: "" });
    };
                    ...
                    <>
                        ...
                        {form.programImagePreview && (
                            <div style={{ border: "double black", width: "60%", display: "flex", flexDirection: "column", alignItems: "center", padding: "1vh 0" }}>
                                ...
                                <img src={form.programImagePreview} alt="preview" style={{ width: "200px", marginTop: "10px", margin: "1vh 0" }} />
