...
const GroupMsgCard = ({ post, trigger, setTrigger }) => {
                ...
                {reply && post?.reply.filter(reply => reply).length !== 0 ?
                    ...
                    <div style={{ background: "grey", display: "flex", flexDirection: "column", alignItems: "center", padding: "1vh 0", margin: "1vh 0" }}>
                        {post?.reply.filter(reply => reply).map(reply => {
                            ...
                            return (
                                <div style={{ width: "90%", background: "rgba(250, 235, 215, 0.960)", margin: "0.5vh 0", display: "flex", flexDirection: "column", border: "solid antiqueWhite", padding: "1vw" }} key={reply?._id}>
                                    <div style={{ width: "100%", display: "flex", justifyContent: "space-between" }}>
