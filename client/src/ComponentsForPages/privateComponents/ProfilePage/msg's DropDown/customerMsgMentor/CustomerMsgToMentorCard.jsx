...
const CustomerMsgToMentorCard = ({ quest, trigger, setTrigger, allUsers }) => {
  ...
  const handleDescriptionKeyDown = (e) => {
    if (e.key === "Tab") {
      ...
      const updatedValue = responseState[e.target.name].substring(0, start) + tabSpaces + responseState[e.target.name].substring(end);
    ...
    <div style={{ width: "90vw", background: "rgba(250, 235, 215, 0.960)", margin: "1vh 0", display: "flex", flexDirection: "column", border: "solid antiqueWhite", padding: "1vw" }} className='responsiveGuestMsg'>
      ...
      {quest?.questionStatus === "sent-response" &&
        ...
        !(
          ...
          <button style={{ background: "goldenRod", width: "100%", height: "5vh", margin: "1vh 0" }} onClick={() => openResponse(quest._id)}>
        ...
        <>
          {quest?.responding === user?._id ? (
            <>
              ...
              <button style={{ background: "red", width: "100%", height: "5vh", margin: "1vh 0", color: "white" }} onClick={() => closeResponse(quest._id)}>
        ...
        {quest?.questionStatus === "sent-response" &&
          ...
          quest.response.slice(1).some((r) => String(r.userId) === String(quest.responding)) && (
            quest?.responding === user?._id ? (
              <>
                {!response ? (
                  <button style={{ background: "goldenRod", width: "100%", height: "5vh", margin: "1vh 0" }} onClick={() => setResponse(true)}>
                    Respond To User
                  </button>
                ) : (
                  <button style={{ background: "red", width: "100%", height: "5vh", margin: "1vh 0", color: "white" }} onClick={() => setResponse(false)}>
                    Cancel This Response
                  </button>
                )}
                ...
                {!response && (
                  <div style={{ background: "green", color: "white", width: "100%", height: "5vh", marginTop: "0.5rem", display: "flex", justifyContent: "center", alignItems: "center" }} onClick={() => handleComplete()}>
                    Complete This Msg
                  </div>
                )}
...
export default CustomerMsgToMentorCard